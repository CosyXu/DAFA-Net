"""
Evaluation metrics for DAFA-Net.
...
def metric_depth(pred, gt, median_scale=False, kitti_mask=False,
                 cityscapes_mask=False, min_depth=0.1, max_depth=80):
    _, h, w = gt.shape
...
class Metric(object):
    ...
    def __init__(self, metric_name, best_compute):
        ...
        self.now_metric = []
        self.computer = []
        if 'depth_synwoodscape' in metric_name:
            self.case_names += [
    ...
    def update_metric(self, outputs, inputs):
        res = []
        if 'depth_synwoodscape' in self.metric_name:
            pred = outputs
            gt = inputs
