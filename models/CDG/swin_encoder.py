"""
Depth encoder model (Swin-transformer) for CDG.

Parts of the code adapted from https://github.com/microsoft/Swin-Transformer.
...
class Path(object):
    ...
    def get_path_of(name):
        if name == 'swin':
            return 'pre_trained/swin/swin_tiny_patch4_window7_224.pth'
        else:
            raise NotImplementedError


def get_orgwintrans_backbone(backbone_name, pretrained):
    if backbone_name == 'orgSwin-T':
        backbone_network = SwinTransformer(embed_dim=96,
                                           depths=[2, 2, 6, 2],
                                           num_heads=[3, 6, 12, 24],
                                           window_size=7,
                                           ape=False,
        ...
        # enc_ch_num = backbone_network.num_features
    elif backbone_name == 'orgSwin-T-repPad-s2':
        backbone_network = SwinTransformer(embed_dim=96,
                                           depths=[2, 2, 6, 2],
                                           num_heads=[3, 6, 12, 24],
                                           window_size=7,
                                           ...
                                           use_checkpoint=False,
                                           embed_stride=2,
                                           embed_pad_mode='replicate')
        # enc_ch_num = backbone_network.num_features
    elif backbone_name == 'orgSwin-T-s2':
        backbone_network = SwinTransformer(embed_dim=96,
                                           depths=[2, 2, 6, 2],
                                           num_heads=[3, 6, 12, 24],
                                           window_size=7,
    ...
    if pretrained:
        if 'Swin' in backbone_name:
            ...
            for k, v in model_dict.items():
                if k in pretrained_dict['model']:
                    if model_dict[k].shape == pretrained_dict['model'][
                            k].shape:
                        model_dict[k] = pretrained_dict['model'][k]
                #     else:
                #         print('change:', k)
                # else:
                #     print('miss:', k)
            backbone_network.load_state_dict(model_dict)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
                 ...
                 norm_layer=None,
                 embed_stride=4,
                 embed_pad_mode='zeros'):
        super().__init__()
        patch_size_num = patch_size
        ...
        self.proj = nn.Conv2d(in_chans,
                              ...
                              stride=embed_stride,
                              padding=(patch_size_num - embed_stride) // 2,
                              padding_mode=embed_pad_mode)
...
class SwinTransformer(nn.Module):
    ...
    Args:
        pretrain_img_size (int): Input image size for training the
        pretrained model,
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                out = x_out.view(-1, H, W,
                                 self.num_ch_enc[i]).permute(0, 3, 1, 2).contiguous()
                outs.append(out)
