"""
Depth decoder model with a super-resolution network for CDG.
...
class ResDecoder(nn.Module):
    ...
    def forward(self, input_features):
        ...
        x = F.interpolate(x, size=input_features[3].shape[2:], mode='bilinear', align_corners=False)
        x = torch.cat((x, input_features[3]), dim=1)
        x = self.upconv_4_1(x)

        x = self.upconv_3_0(x)
        x = self.shuffle_conv_3_0(x)
        x = F.interpolate(x, size=input_features[2].shape[2:], mode='bilinear', align_corners=False)
        x = torch.cat((x, input_features[2]), dim=1)
        x_3 = self.upconv_3_1(x)

        x = self.upconv_2_0(x_3)
        x = self.shuffle_conv_2_0(x)
        x = F.interpolate(x, size=input_features[1].shape[2:], mode='bilinear', align_corners=False)
        x = torch.cat((x, input_features[1]), dim=1)
        x_2 = self.upconv_2_1(x)

        x = self.upconv_1_0(x_2)
        x = self.shuffle_conv_1_0(x)
        x = F.interpolate(x, size=input_features[0].shape[2:], mode='bilinear', align_corners=False)
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            outputs[("disp", 3)] = self.sigmoid(self.shuffle_conv_3(x_3))
            outputs[("disp", 2)] = self.sigmoid(self.shuffle_conv_2(x_2))
            outputs[("disp", 1)] = self.sigmoid(self.shuffle_conv_1(x_1))
            outputs[("disp", 0)] = self.sigmoid(self.normconv_0(x))
            return outputs
...
class SwinDecoder(nn.Module):
    def __init__(self, num_ch_enc, num_output_channels=1):
        ...
        self.upconv_3_0 = convblock(self.num_ch_enc[-1], self.num_ch_dec[3])
        self.upconv_3_1 = convblock(self.num_ch_dec[3] + self.num_ch_enc[2], self.num_ch_dec[3])
